...
class sale_configuration_settings(models.TransientModel):
    ...
    def get_values(self):
        ...
        referral_commission_account= int(self.env['ir.config_parameter'].sudo().get_param('sale_referral_commission_ad.referral_commission_account'))
        res.update(referral_commission_account = referral_commission_account)
        return res

    def set_values(self):
        super(sale_configuration_settings, self).set_values()
        self.env['ir.config_parameter'].sudo().set_param('sale_referral_commission_ad.referral_commission_account', self.referral_commission_account.id)